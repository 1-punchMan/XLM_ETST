...
def check_data_params(params):
    ...
    params.pc_steps = [tuple(s.split('-'))
                       for s in params.pc_steps.split(',') if len(s) > 0]
    ...
    # required_para_train = set(params.clm_steps + params.mlm_steps + params.pc_steps + params.mt_steps + params.unsclts_steps)
    ########################################
    ...
    assert all([all([os.path.isfile(p1) and os.path.isfile(p2)
                     for p1, p2 in paths.values()]) for paths in params.para_dataset.values()])

    # check that we can evaluate on BLEU
    assert params.eval_bleu is False or len(
        params.mt_steps + params.bt_steps) > 0
...
def seq2seq_load_data(params):
    ...
    # parallel datasets
    seq2seq_load_para_data(params, data)
...
def seq2seq_load_binarized(path, params, split):
    """
    Load a binarized dataset.
    """
    assert path.endswith('.pth')
    ...
    logger.info("Loading data from %s ..." % path)
    data = torch.load(path)
    data = seq2seq_process_binarized(data, params, split)
    return data


def seq2seq_process_binarized(data, params, split):
    """
    Process a binarized dataset and log main statistics.
    """
    ...
    if split is None:
        ...
        dico = data['dico']
        max_vocab = params.src_max_vocab
        min_count = params.src_min_count
    elif split == 'tgt':
        dico = data['dico']
        max_vocab = params.tgt_max_vocab
        min_count = params.tgt_min_count
    ...
    if max_vocab != -1:
        assert max_vocab > 0
        logger.info("Selecting %i most frequent words ..." % max_vocab)
        dico.max_vocab(max_vocab)
        data['sentences'][data['sentences'] >=
                          max_vocab] = dico.index(UNK_WORD)
        unk_count = (data['sentences'] == dico.index(UNK_WORD)).sum()
        logger.info("Now %i unknown words covering %.2f%% of the data."
                    % (unk_count, 100. * unk_count / (len(data['sentences']) - len(data['positions']))))
    if min_count > 0:
...
def seq2seq_set_dico_parameters(params, data, dico, split):
    """
    Update dictionary parameters.
    """
    ...
    if split is None:
        ...
        if 'src_dico' in data:
            assert data['src_dico'] == dico
        else:
            data['src_dico'] = dico
    elif split == 'tgt':
        if 'tgt_dico' in data:
            assert data['tgt_dico'] == dico
        else:
            data['tgt_dico'] = dico
    ...
    if split is None:
        ...
        params.src_n_words = n_words
        
        params.bos_index = bos_index
        params.eos_index = eos_index
        params.pad_index = pad_index
        params.unk_index = unk_index
        params.src_mask_index = mask_index

    elif split == 'tgt':
        params.tgt_n_words = n_words
        
        params.bos_index = bos_index
        params.eos_index = eos_index
        params.pad_index = pad_index
        params.unk_index = unk_index
        params.tgt_mask_index = mask_index


def seq2seq_load_para_data(params, data):
    """
    Load parallel data.
    """
    data['para'] = {}
    ...
    for src, tgt in params.para_dataset.keys():
        ...
        for splt in ['train', 'valid', 'test']:
            ...
            src_data = seq2seq_load_binarized(src_path, params, 'src')
            tgt_data = seq2seq_load_binarized(tgt_path, params, 'tgt')

            # update dictionary parameters
            seq2seq_set_dico_parameters(params, data, src_data['dico'], 'src')
            seq2seq_set_dico_parameters(params, data, tgt_data['dico'], 'tgt')
