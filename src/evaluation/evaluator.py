...
BLEU_SCRIPT_PATH = os.path.join(os.path.abspath(
    os.path.dirname(__file__)), 'multi-bleu.perl')
...
class Evaluator(object):
    ...
    def get_iterator(self, data_set, lang1, lang2=None, stream=False):
        ...
        if len(self.params.langs) > 30:
            ...
            eval_lgs = set(["ar", "bg", "de", "el", "en", "es",
                            "fr", "hi", "ru", "sw", "th", "tr", "ur", "vi", "zh"])
            subsample = 10 if (
                data_set == 'test' or lang1 not in eval_lgs) else 5
            n_sentences = 600 if (
                data_set == 'test' or lang1 not in eval_lgs) else 1500
    ...
    def evaluate_clm(self, scores, data_set, lang1, lang2):
        ...
        if eval_memory:
            for mem_name, mem_att in all_mem_att.items():
                eval_memory_usage(scores, '%s_%s_%s' % (
...
class EncDecEvaluator(Evaluator):
    ...
    def evaluate_mt(self, scores, data_set, lang1, lang2, eval_bleu, eval_rouge):
        ...
        if eval_bleu or eval_rouge:  # edited by chiamin. origin
            hypothesis = []
        ...
        if eval_memory:
            for mem_name, mem_att in all_mem_att.items():
                eval_memory_usage(scores, '%s_%s-%s_%s' % (data_set,
                                                           lang1, lang2, mem_name), mem_att, params.mem_size)

        # compute BLEU
        if eval_bleu:

            # hypothesis / reference paths
            hyp_name = 'hyp{0}.{1}-{2}.{3}.txt'.format(
                scores['epoch'], lang1, lang2, data_set)
        ...
        if eval_rouge:
            ...
            if os.path.isfile(hyp_path) is False:
                # export sentences to hypothesis file / restore BPE segmentation
                with open(hyp_path, 'w', encoding='utf-8') as f:
                    f.write('\n'.join(hypothesis) + '\n')
            restore_segmentation(hyp_path)
            ...
            # evaluate rouge F1 score
            # if params.eval_only:
            rouge_f1 = eval_rouge_f1(ref_path, hyp_path)
...
def eval_rouge_f1(ref, hyp):
    ...
    #     'ROUGE-3': scores['rouge-3']['f']  * 100.,
    #     'ROUGE-4': scores['rouge-4']['f']  * 100.,
...
class XLMCLTSEncDecEvaluator(Evaluator):
    ...
    def __init__(self, trainer, data, params):
        ...
        """
        super().__init__(trainer, data, params)
        self.xencoder = trainer.xencoder
        self.encoder = trainer.encoder
        self.decoder = trainer.decoder
    ...
    def evaluate_mt(self, scores, data_set, lang1, lang2, eval_bleu, eval_rouge):
        ...
        for batch in self.get_iterator(data_set, lang1, lang2):
            ...
            # encode source sentence
            enc1 = encoder('fwd', x=x1, lengths=len1, langs=langs1, causal=False, xencoder=_xencoder)
            enc1 = enc1.transpose(0, 1)
            enc1 = enc1.half() if params.fp16 else enc1

            # decode target sentence
            dec2 = decoder('fwd', x=x2, lengths=len2, langs=langs2, causal=True, src_enc=enc1, src_len=len1, xencoder=_xencoder)
...
class Seq2SeqEvaluator(object):
    ...
    def __init__(self, trainer, data, params):
        ...
        self.trainer = trainer
        self.data = data
        self.src_dico = data['src_dico']
        self.tgt_dico = data['tgt_dico']
        self.params = params
        self.memory_list = trainer.memory_list
    ...
    def create_reference_files(self):
        ...
        for (lang1, lang2), v in self.data['para'].items():
            ...
            for data_set in ['valid', 'test']:
                ...
                for (sent1, len1), (sent2, len2) in self.get_iterator(data_set, lang1, lang2):
                    lang1_txt.extend(convert_to_text(
                        sent1, len1, self.src_dico, params))
                    lang2_txt.extend(convert_to_text(
                        sent2, len2, self.tgt_dico, params))
    ...
    def mask_out(self, x, lengths, rng):
        ...
        assert 0 <= x.min() <= x.max() < params.src_n_words
        assert x.size() == (slen, bs)
        assert pred_mask.size() == (slen, bs)
    ...
    def evaluate_mt(self, scores, data_set, lang1, lang2, eval_bleu):
        ...
        for batch in self.get_iterator(data_set, lang1, lang2):
            ...
            if eval_bleu:
                ...
                # 所以 en 變成 src ， zh 變成 tgt  ----->  src_dico (en)
                hypothesis.extend(convert_to_text(
                    generated, lengths, self.src_dico, params))
