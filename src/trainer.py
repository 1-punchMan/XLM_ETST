...
class Trainer(object):
    ...
    def __init__(self, data, params):
        ...
        counts = np.array(list(self.data['dico'].counts.values()))  # tgt_dico
    ...
    def set_parameters(self):
        ...
        if params.use_xencoder:
            self.parameters['xencoder'] = [p for k, p in getattr(self, 'xencoder').named_parameters() if p.requires_grad]
        ############################
...
class EncDecTrainer(Trainer):
    ...
    def mt_step(self, lang1, lang2, lambda_coeff):
        ...
        # x1_word = [self.data['dico'].id2word[x.item()] for x in x1.transpose(0, 1)[0]]
        # x2_word = [self.data['dico'].id2word[x.item()] for x in x2.transpose(0, 1)[0]]
        # print(x1_word)
        # print(x2_word)
        ...
        # _word = [self.data['dico'].id2word[x.item()] for x in _.transpose(0, 1)[0]]
        # print(_word)
        # exit()
...
class XLMCLTSEncDecTrainer(Trainer):
    ...
    def __init__(self, xencoder, encoder, decoder, data, params):

        self.MODEL_NAMES = ['xencoder', 'encoder', 'decoder']
    ...
    def mt_step(self, lang1, lang2, lambda_coeff):
        ...
        # decode target sentence
        dec2 = self.decoder('fwd', x=x2, lengths=len2, langs=langs2, causal=True, src_enc=enc1, src_len=len1, xencoder=self.xencoder)
        # dec2 = self.decoder('fwd', x=x2, lengths=len2, langs=langs2, causal=True, src_enc=enc1, src_len=len1)
    ...
    def bt_step(self, lang1, lang2, lang3, lambda_coeff):
        ...
        assert lang1 == lang3 and lang1 != lang2 and lang2 is not None
        params = self.params
        _xencoder = self.xencoder.module if params.multi_gpu else self.xencoder
        _encoder = self.encoder.module if params.multi_gpu else self.encoder
        _decoder = self.decoder.module if params.multi_gpu else self.decoder
        ...
        with torch.no_grad():
            ...
            # encode source sentence and translate it
            enc1 = _encoder('fwd', x=x1, lengths=len1, langs=langs1, causal=False, xencoder=_xencoder)
            enc1 = enc1.transpose(0, 1)
            x2, len2 = _decoder.generate(enc1, len1, lang2_id, max_len=int(1.3 * len1.max().item() + 5), xencoder=_xencoder)
            langs2 = x2.clone().fill_(lang2_id)
        ...
        # encode generate sentence
        enc2 = self.encoder('fwd', x=x2, lengths=len2, langs=langs2, causal=False, xencoder=self.xencoder)
        enc2 = enc2.transpose(0, 1)
        ...
        # decode original sentence
        dec3 = self.decoder('fwd', x=x1, lengths=len1, langs=langs1, causal=True, src_enc=enc2, src_len=len2, xencoder=self.xencoder)
...
class Seq2SeqTrainer(Trainer):
    ...
    def __init__(self, data, params):
        ...
        # probabilty to predict a word
        counts = np.array(list(self.data['tgt_dico'].counts.values()))
        params.mask_scores = np.maximum(counts, 1) ** -params.sample_alpha
        params.mask_scores[params.pad_index] = 0  # do not predict <PAD> index
        params.mask_scores[counts == 0] = 0       # do not predict special tokens
    ...
    def save_checkpoint(self, name, include_optimizers=True):
        ...
        data['src_dico_id2word'] = self.data['src_dico'].id2word
        data['src_dico_word2id'] = self.data['src_dico'].word2id
        data['src_dico_counts'] = self.data['src_dico'].counts
        data['tgt_dico_id2word'] = self.data['tgt_dico'].id2word
        data['tgt_dico_word2id'] = self.data['tgt_dico'].word2id
        data['tgt_dico_counts'] = self.data['tgt_dico'].counts
        data['params'] = {k: v for k, v in self.params.__dict__.items()}
...
# class Seq2SeqEncDecTrainer(Seq2SeqTrainer):
